...
| 标头 | 描述 |
|------|------|
| `X-Glue-Authentication` | Base64 编码的 JWT，包含用户 ID、会话信息、角色和身份验证方法 |
...
后端服务使用这些标头识别调用者并执行授权。`X-Glue-Authentication` JWT 使用仅 Glue2 和受信任后端已知的共享密钥进行签名。
...
Glue2 将流量分布到每个服务的多个后端实例。如果某个实例不健康（健康检查失败或返回错误），Glue2 将其从轮换中移除并在健康实例上重试。健康检查每 10 秒执行一次。您可以通过 `/v0/status` 端点监控后端健康状态。
