...
- **项目**：密钥仅对所属项目的请求有效。
- **主机**：限制到特定的虚拟主机。
- **路径**：限制到路径前缀模式（例如 `/api/v0/lambdas/*`）。
- **方法**：限制到特定的 HTTP 方法（`GET`、`POST` 等）。

如果设置了任何范围，则所有条件必须匹配。未设置范围的密钥拥有完全访问权限。
...
当提供有效的 API 密钥时，Glue2 会将 `X-Glue-Authentication` 标头注入代理请求中，使后端能够识别调用者。
...
方法          凭证                    典型用途              范围
──────────────────────────────────────────────────────────────────────
API 密钥     Api-Key + Api-Secret    机器对机器            项目/主机/路径/方法
...
- **最小权限原则**：将每个密钥的范围限制到所需的最小主机、路径和方法集。
